...
class EscortalligatorScraper(ScraperPrototype):
    ...
    def stop_scraper(self):
        """
        Gracefully stops the scraper by quitting the WebDriver and resetting variables.
        """
        try:
            if self.driver:
    ...
    def CLEAN_format_data_to_excel(self) -> None:
        ...
        titled_columns = pd.DataFrame({
            ...
            # ------- keyword stats
            'Keywords-found': self.keywords_found,
            'Number-of-keywords-found': self.number_of_keywords_found  # Correct attribute name
        })
